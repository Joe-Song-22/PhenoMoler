...
def compute_tanimoto_similarity(smiles1, smiles2):
    try:
        ...
        return 0.0
def extract_scaffold_selfies_2(decoder_inputs, vocab, pad_token=0):
    """
    提取 decoder_inputs 的核心骨架 SMILES，并转换为 SELFIES，同时返回骨架 SELFIES 对应的 token id。
    ...
    返回:
        scaffold_selfies_list: list of scaffold SELFIES 字符串
        scaffold_smiles_list: list of scaffold SMILES 字符串
        scaffold_token_ids_list: list of list[int]，骨架 SELFIES 对应的 token id
    """
    batch_size, seq_len = decoder_inputs.shape
    scaffold_smiles_list = []
    scaffold_selfies_list = []
    scaffold_token_ids_list = []

    inv_vocab = {v: k for k, v in vocab.items()}  # 索引 -> 片段
    ...
    for i in range(batch_size):
        ...
        selfies_str = ''.join([t for t in selfies_tokens if t not in ('[START]', '[END]', '[PAD]')])
        ...
        try:
            ...
            if mol is None:
                scaffold_smiles_list.append('')
                scaffold_selfies_list.append('')
                scaffold_token_ids_list.append([])
                continue
            ...
            # 骨架 SMILES -> SELFIES
            scaffold_selfies = sf.encoder(scaffold_smiles)
            scaffold_selfies_list.append(scaffold_selfies)

            # SELFIES -> token ids
            scaffold_ids = []
            tmp = ''
            for ch in scaffold_selfies:
                tmp += ch
                if ch == ']':  # SELFIES token 以 "]" 结束
                    scaffold_ids.append(vocab.get(tmp, pad_token))
                    tmp = ''
            scaffold_token_ids_list.append(scaffold_ids)

        except Exception as e:
            print(f"[Warning] 行 {i} 处理失败: {e}")
            scaffold_smiles_list.append('')
            scaffold_selfies_list.append('')
            scaffold_token_ids_list.append([])

    return scaffold_selfies_list, scaffold_smiles_list, scaffold_token_ids_list
